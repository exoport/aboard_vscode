...
- [x] **ANSWERED, and the answer is no: a cropped image region cannot reach the clipboard from the panel.** (2026-08-28) The board's markup renderer copies a rectangle as a PNG, and it works in a plain browser. In here it does not, and it cannot be made to: Chromium refuses with *"The Clipboard API has been blocked because of a permissions policy applied to the current document"*. `media/panel.html` does send `allow="clipboard-write"`, which is necessary and not sufficient — the webview document above it must hold the permission before it can delegate it, and **VS Code exposes no way to ask for that**. Checked against `@types/vscode` rather than assumed: `WebviewOptions` offers `enableScripts`, `enableForms`, `enableCommandUris` and `localResourceRoots` and no permission field, and `vscode.env.clipboard` is `readText()` / `writeText(string)` — **text only, no image**. The board handles it: the refusal is reported and the crop is offered as a picture and as **Add it to this tab**, which needs no permission and is what the feature was for. Right-click on that picture is a browser answer and NOT a panel one — the host owns the context menu here and offers no *Copy image*, which cost one more round of "it appears to do nothing". The one route left unexplored is the extension shelling out to a platform clipboard tool (`xclip`, `wl-copy`, `pbcopy`, PowerShell) with a temp file; it is not built, and it would be this repository's first platform-specific code and first dependency on a binary outside VS Code.
