...
- [ ] **A cropped image region actually reaches the clipboard from the panel.** The board's markup renderer copies a rectangle as a PNG. In here the board is a cross-origin frame, so `media/panel.html` sends `allow="clipboard-write"` — and whether VS Code's own webview delegates that permission to a NESTED frame is not something any test in this repository can answer. The board reports the refusal in its own status line rather than swallowing it, so this row is answerable by pressing the button and reading the line. It works in a plain browser (asserted in the aboard repo's suite against a real Chromium).
