...
- [why-the-shell-is-probed.md](why-the-shell-is-probed.md) — `?chrome=notabs` is silently ignored by an older board. From aboard v0.2.0 `/capabilities` declares it and is read first; before that the manifest says nothing, and testing the feature beats testing a proxy for it.
