...
- [ ] **On a light editor theme, the panel opens light — no dark first frame.** Against aboard v0.2.0 or later, under `aboard.theme: follow`: switch VS Code to a light theme, close the panel, open it, and watch the first frame. The URL is asserted in `test/model.test.ts` and the kind mapping in `test/theme.test.ts`; that the board then paints light before `load` is aboard's browser suite. What only a host can show is the three meeting — `activeColorTheme.kind` read at panel creation, the webview loading that URL, and no flash. A small step from the board's light ground to the editor's is expected and is not this row failing. Worth checking in the same sitting: a sidebar click still navigates after switching VS Code between light and dark with the panel open, which is the case the fixed-for-the-panel's-lifetime parameter exists for.
