...
async function rows(): Promise<{ vscode: typeof import('./vscode-stub'); rows: RenderedRow[]; dispose(): void }> {
  ...
  const found = await until('the tree to list the board’s tabs', 10_000, () => {
    const tabs = running.vscode.probe.rows.filter((r) => /^ab\d+$/.test(r.description ?? ''));
...
describe('the two copy commands', { timeout: 30_000 }, () => {
  it('copies a reference: the name, with the id beside it', async () => {
    ...
    try {
      const row = found.find((r) => r.description === 'ab1');
      ...
      await copy(row.node);
      assert.deepEqual(vscode.probe.clipboard, ['Migration review (ab1)']);
    } finally {
      dispose();
    }
  });
  ...
  it('copies a link: the deep link the board’s own menu builds', async () => {
    const { vscode, rows: found, dispose } = await rows();
    try {
      const row = found.find((r) => r.description === 'ab71');
      assert.ok(row);
      ...
      await copy(row.node);
      assert.deepEqual(vscode.probe.clipboard, [`http://127.0.0.1:${board.port}/#tab=ab71`]);
    } finally {
      dispose();
    }
  });
  ...
  it('copies the bare id for a tab with no name', async () => {
    const { vscode, rows: found, dispose } = await rows();
    try {
      const row = found.find((r) => r.description === 'ab99');
      ...
      await vscode.probe.commands.get('aboard.copyReference')!(row.node);
      // Not `(unnamed) (ab99)`: the board's placeholder is not a name.
      assert.deepEqual(vscode.probe.clipboard, ['ab99']);
    } finally {
      dispose();
    }
  });
  ...
  it('still copies the id on its own', async () => {
    ...
    try {
      ...
      assert.deepEqual(vscode.probe.clipboard, ['ab71']);
    } finally {
      dispose();
    }
  });
